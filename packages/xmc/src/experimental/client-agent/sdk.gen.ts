// This file is auto-generated by @hey-api/openapi-ts

import {
  type Options as ClientOptions,
  type TDataShape,
  type Client,
  formDataBodySerializer,
} from '@hey-api/client-fetch';
import type { experimental_Agent } from './types.gen';
import { client as _heyApiClient } from './client.gen';

type JobsListOperationsError = experimental_Agent.JobsListOperationsError;

type JobsListOperationsData = experimental_Agent.JobsListOperationsData;

type JobsGetJobError = experimental_Agent.JobsGetJobError;

type JobsGetJobData = experimental_Agent.JobsGetJobData;

type JobsRevertJobError = experimental_Agent.JobsRevertJobError;

type JobsRevertJobData = experimental_Agent.JobsRevertJobData;

type BriefUpdateBriefError = experimental_Agent.BriefUpdateBriefError;

type BriefUpdateBriefResponse = experimental_Agent.BriefUpdateBriefResponse;

type BriefUpdateBriefData = experimental_Agent.BriefUpdateBriefData;

type GetBriefByIdError = experimental_Agent.GetBriefByIdError;

type GetBriefByIdResponse = experimental_Agent.GetBriefByIdResponse;

type GetBriefByIdData = experimental_Agent.GetBriefByIdData;

type GetBriefTypeByIdError = experimental_Agent.GetBriefTypeByIdError;

type GetBriefTypeByIdResponse = experimental_Agent.GetBriefTypeByIdResponse;

type GetBriefTypeByIdData = experimental_Agent.GetBriefTypeByIdData;

type BriefsGenerateBriefDeprecatedError = experimental_Agent.BriefsGenerateBriefDeprecatedError;

type BriefsGenerateBriefDeprecatedResponse =
  experimental_Agent.BriefsGenerateBriefDeprecatedResponse;

type BriefsGenerateBriefDeprecatedData = experimental_Agent.BriefsGenerateBriefDeprecatedData;

type BriefsGenerateBriefError = experimental_Agent.BriefsGenerateBriefError;

type BriefsGenerateBriefResponse = experimental_Agent.BriefsGenerateBriefResponse;

type BriefsGenerateBriefData = experimental_Agent.BriefsGenerateBriefData;

type BriefCreateBriefError = experimental_Agent.BriefCreateBriefError;

type BriefCreateBriefResponse = experimental_Agent.BriefCreateBriefResponse;

type BriefCreateBriefData = experimental_Agent.BriefCreateBriefData;

type ListBriefsError = experimental_Agent.ListBriefsError;

type ListBriefsResponse2 = experimental_Agent.ListBriefsResponse2;

type ListBriefsData = experimental_Agent.ListBriefsData;

type ListBriefTypesError = experimental_Agent.ListBriefTypesError;

type ListBriefTypesResponse2 = experimental_Agent.ListBriefTypesResponse2;

type ListBriefTypesData = experimental_Agent.ListBriefTypesData;

type BrandkitsGetBrandkitByIdError = experimental_Agent.BrandkitsGetBrandkitByIdError;

type BrandkitsGetBrandkitByIdResponse = experimental_Agent.BrandkitsGetBrandkitByIdResponse;

type BrandkitsGetBrandkitByIdData = experimental_Agent.BrandkitsGetBrandkitByIdData;

type BrandkitsListBrandkitsError = experimental_Agent.BrandkitsListBrandkitsError;

type BrandkitsListBrandkitsResponse = experimental_Agent.BrandkitsListBrandkitsResponse;

type BrandkitsListBrandkitsData = experimental_Agent.BrandkitsListBrandkitsData;

type FlowsSetupVariantError = experimental_Agent.FlowsSetupVariantError;

type FlowsSetupVariantResponse = experimental_Agent.FlowsSetupVariantResponse;

type FlowsSetupVariantData = experimental_Agent.FlowsSetupVariantData;

type FlowsGetVariantError = experimental_Agent.FlowsGetVariantError;

type FlowsGetVariantResponse = experimental_Agent.FlowsGetVariantResponse;

type FlowsGetVariantData = experimental_Agent.FlowsGetVariantData;

type FlowsResetComponentVariantError = experimental_Agent.FlowsResetComponentVariantError;

type FlowsResetComponentVariantResponse = experimental_Agent.FlowsResetComponentVariantResponse;

type FlowsResetComponentVariantData = experimental_Agent.FlowsResetComponentVariantData;

type FlowsGetFlowDefinitionError = experimental_Agent.FlowsGetFlowDefinitionError;

type FlowsGetFlowDefinitionResponse = experimental_Agent.FlowsGetFlowDefinitionResponse;

type FlowsGetFlowDefinitionData = experimental_Agent.FlowsGetFlowDefinitionData;

type FlowsListFlowDefinitionsByPageError = experimental_Agent.FlowsListFlowDefinitionsByPageError;

type FlowsListFlowDefinitionsByPageResponse =
  experimental_Agent.FlowsListFlowDefinitionsByPageResponse;

type FlowsListFlowDefinitionsByPageData = experimental_Agent.FlowsListFlowDefinitionsByPageData;

type PersonalizationHideComponentOnDefaultPageError =
  experimental_Agent.PersonalizationHideComponentOnDefaultPageError;

type PersonalizationHideComponentOnDefaultPageResponse =
  experimental_Agent.PersonalizationHideComponentOnDefaultPageResponse;

type PersonalizationHideComponentOnDefaultPageData =
  experimental_Agent.PersonalizationHideComponentOnDefaultPageData;

type PersonalizationUpdatePersonalizationVersionError =
  experimental_Agent.PersonalizationUpdatePersonalizationVersionError;

type PersonalizationUpdatePersonalizationVersionResponse =
  experimental_Agent.PersonalizationUpdatePersonalizationVersionResponse;

type PersonalizationUpdatePersonalizationVersionData =
  experimental_Agent.PersonalizationUpdatePersonalizationVersionData;

type PersonalizationGetConditionTemplateByIdError =
  experimental_Agent.PersonalizationGetConditionTemplateByIdError;

type PersonalizationGetConditionTemplateByIdResponse =
  experimental_Agent.PersonalizationGetConditionTemplateByIdResponse;

type PersonalizationGetConditionTemplateByIdData =
  experimental_Agent.PersonalizationGetConditionTemplateByIdData;

type PersonalizationGetConditionTemplatesError =
  experimental_Agent.PersonalizationGetConditionTemplatesError;

type PersonalizationGetConditionTemplatesResponse =
  experimental_Agent.PersonalizationGetConditionTemplatesResponse;

type PersonalizationGetConditionTemplatesData =
  experimental_Agent.PersonalizationGetConditionTemplatesData;

type PersonalizationGetPersonalizationVersionsByPageError =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageError;

type PersonalizationGetPersonalizationVersionsByPageResponse =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageResponse;

type PersonalizationGetPersonalizationVersionsByPageData =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageData;

type PersonalizationGetPersonalizationVersionsByPageV1Error =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageV1Error;

type PersonalizationGetPersonalizationVersionsByPageV1Response =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageV1Response;

type PersonalizationGetPersonalizationVersionsByPageV1Data =
  experimental_Agent.PersonalizationGetPersonalizationVersionsByPageV1Data;

type PersonalizationCreatePersonalizationVersionError =
  experimental_Agent.PersonalizationCreatePersonalizationVersionError;

type PersonalizationCreatePersonalizationVersionResponse =
  experimental_Agent.PersonalizationCreatePersonalizationVersionResponse;

type PersonalizationCreatePersonalizationVersionData =
  experimental_Agent.PersonalizationCreatePersonalizationVersionData;

type PersonalizationCreatePersonalizationVersionV1Error =
  experimental_Agent.PersonalizationCreatePersonalizationVersionV1Error;

type PersonalizationCreatePersonalizationVersionV1Response =
  experimental_Agent.PersonalizationCreatePersonalizationVersionV1Response;

type PersonalizationCreatePersonalizationVersionV1Data =
  experimental_Agent.PersonalizationCreatePersonalizationVersionV1Data;

type ExperimentsUpdateAbTestError = experimental_Agent.ExperimentsUpdateAbTestError;

type ExperimentsUpdateAbTestResponse = experimental_Agent.ExperimentsUpdateAbTestResponse;

type ExperimentsUpdateAbTestData = experimental_Agent.ExperimentsUpdateAbTestData;

type ExperimentsCreateComponentAbTestError =
  experimental_Agent.ExperimentsCreateComponentAbTestError;

type ExperimentsCreateComponentAbTestResponse =
  experimental_Agent.ExperimentsCreateComponentAbTestResponse;

type ExperimentsCreateComponentAbTestData = experimental_Agent.ExperimentsCreateComponentAbTestData;

type EnvironmentsListLanguagesError = experimental_Agent.EnvironmentsListLanguagesError;

type EnvironmentsListLanguagesResponse = experimental_Agent.EnvironmentsListLanguagesResponse;

type EnvironmentsListLanguagesData = experimental_Agent.EnvironmentsListLanguagesData;

type AssetsUpdateAssetError = experimental_Agent.AssetsUpdateAssetError;

type AssetsUpdateAssetResponse = experimental_Agent.AssetsUpdateAssetResponse;

type AssetsUpdateAssetData = experimental_Agent.AssetsUpdateAssetData;

type AssetsGetAssetInformationError = experimental_Agent.AssetsGetAssetInformationError;

type AssetsGetAssetInformationResponse = experimental_Agent.AssetsGetAssetInformationResponse;

type AssetsGetAssetInformationData = experimental_Agent.AssetsGetAssetInformationData;

type AssetsSearchAssetsError = experimental_Agent.AssetsSearchAssetsError;

type AssetsSearchAssetsResponse = experimental_Agent.AssetsSearchAssetsResponse;

type AssetsSearchAssetsData = experimental_Agent.AssetsSearchAssetsData;

type AssetsUploadAssetError = experimental_Agent.AssetsUploadAssetError;

type AssetsUploadAssetResponse = experimental_Agent.AssetsUploadAssetResponse;

type AssetsUploadAssetData = experimental_Agent.AssetsUploadAssetData;

type ComponentsGetComponentError = experimental_Agent.ComponentsGetComponentError;

type ComponentsGetComponentResponse = experimental_Agent.ComponentsGetComponentResponse;

type ComponentsGetComponentData = experimental_Agent.ComponentsGetComponentData;

type ComponentsListComponentsError = experimental_Agent.ComponentsListComponentsError;

type ComponentsListComponentsResponse = experimental_Agent.ComponentsListComponentsResponse;

type ComponentsListComponentsData = experimental_Agent.ComponentsListComponentsData;

type ComponentsSearchComponentDatasourcesError =
  experimental_Agent.ComponentsSearchComponentDatasourcesError;

type ComponentsSearchComponentDatasourcesResponse =
  experimental_Agent.ComponentsSearchComponentDatasourcesResponse;

type ComponentsSearchComponentDatasourcesData =
  experimental_Agent.ComponentsSearchComponentDatasourcesData;

type ComponentsCreateComponentDatasourceError =
  experimental_Agent.ComponentsCreateComponentDatasourceError;

type ComponentsCreateComponentDatasourceResponse =
  experimental_Agent.ComponentsCreateComponentDatasourceResponse;

type ComponentsCreateComponentDatasourceData =
  experimental_Agent.ComponentsCreateComponentDatasourceData;

type ContentListAvailableInsertoptionsError =
  experimental_Agent.ContentListAvailableInsertoptionsError;

type ContentListAvailableInsertoptionsResponse =
  experimental_Agent.ContentListAvailableInsertoptionsResponse;

type ContentListAvailableInsertoptionsData =
  experimental_Agent.ContentListAvailableInsertoptionsData;

type ContentGetContentItemByPathError = experimental_Agent.ContentGetContentItemByPathError;

type ContentGetContentItemByPathResponse = experimental_Agent.ContentGetContentItemByPathResponse;

type ContentGetContentItemByPathData = experimental_Agent.ContentGetContentItemByPathData;

type ContentUpdateContentError = experimental_Agent.ContentUpdateContentError;

type ContentUpdateContentResponse = experimental_Agent.ContentUpdateContentResponse;

type ContentUpdateContentData = experimental_Agent.ContentUpdateContentData;

type ContentGetContentItemByIdError = experimental_Agent.ContentGetContentItemByIdError;

type ContentGetContentItemByIdResponse = experimental_Agent.ContentGetContentItemByIdResponse;

type ContentGetContentItemByIdData = experimental_Agent.ContentGetContentItemByIdData;

type ContentDeleteContentError = experimental_Agent.ContentDeleteContentError;

type ContentDeleteContentResponse = experimental_Agent.ContentDeleteContentResponse;

type ContentDeleteContentData = experimental_Agent.ContentDeleteContentData;

type ContentCreateContentItemError = experimental_Agent.ContentCreateContentItemError;

type ContentCreateContentItemResponse = experimental_Agent.ContentCreateContentItemResponse;

type ContentCreateContentItemData = experimental_Agent.ContentCreateContentItemData;

type PagesGetAllowedComponentsByPlaceholderError =
  experimental_Agent.PagesGetAllowedComponentsByPlaceholderError;

type PagesGetAllowedComponentsByPlaceholderResponse =
  experimental_Agent.PagesGetAllowedComponentsByPlaceholderResponse;

type PagesGetAllowedComponentsByPlaceholderData =
  experimental_Agent.PagesGetAllowedComponentsByPlaceholderData;

type PagesGetPageError = experimental_Agent.PagesGetPageError;

type PagesGetPageResponse = experimental_Agent.PagesGetPageResponse;

type PagesGetPageData = experimental_Agent.PagesGetPageData;

type PagesGetPageTemplateByIdError = experimental_Agent.PagesGetPageTemplateByIdError;

type PagesGetPageTemplateByIdResponse = experimental_Agent.PagesGetPageTemplateByIdResponse;

type PagesGetPageTemplateByIdData = experimental_Agent.PagesGetPageTemplateByIdData;

type PagesGetPagePreviewUrlError = experimental_Agent.PagesGetPagePreviewUrlError;

type PagesGetPagePreviewUrlResponse = experimental_Agent.PagesGetPagePreviewUrlResponse;

type PagesGetPagePreviewUrlData = experimental_Agent.PagesGetPagePreviewUrlData;

type PagesGetPageHtmlError = experimental_Agent.PagesGetPageHtmlError;

type PagesGetPageHtmlResponse = experimental_Agent.PagesGetPageHtmlResponse;

type PagesGetPageHtmlData = experimental_Agent.PagesGetPageHtmlData;

type PagesGetPageScreenshotError = experimental_Agent.PagesGetPageScreenshotError;

type PagesGetPageScreenshotResponse = experimental_Agent.PagesGetPageScreenshotResponse;

type PagesGetPageScreenshotData = experimental_Agent.PagesGetPageScreenshotData;

type PagesGetPagePathByLiveUrlError = experimental_Agent.PagesGetPagePathByLiveUrlError;

type PagesGetPagePathByLiveUrlResponse = experimental_Agent.PagesGetPagePathByLiveUrlResponse;

type PagesGetPagePathByLiveUrlData = experimental_Agent.PagesGetPagePathByLiveUrlData;

type PagesSearchSiteError = experimental_Agent.PagesSearchSiteError;

type PagesSearchSiteResponse = experimental_Agent.PagesSearchSiteResponse;

type PagesSearchSiteData = experimental_Agent.PagesSearchSiteData;

type PagesSetComponentDatasourceError = experimental_Agent.PagesSetComponentDatasourceError;

type PagesSetComponentDatasourceResponse = experimental_Agent.PagesSetComponentDatasourceResponse;

type PagesSetComponentDatasourceData = experimental_Agent.PagesSetComponentDatasourceData;

type PagesAddComponentOnPageError = experimental_Agent.PagesAddComponentOnPageError;

type PagesAddComponentOnPageResponse = experimental_Agent.PagesAddComponentOnPageResponse;

type PagesAddComponentOnPageData = experimental_Agent.PagesAddComponentOnPageData;

type PagesGetComponentsOnPageError = experimental_Agent.PagesGetComponentsOnPageError;

type PagesGetComponentsOnPageResponse = experimental_Agent.PagesGetComponentsOnPageResponse;

type PagesGetComponentsOnPageData = experimental_Agent.PagesGetComponentsOnPageData;

type PagesAddLanguageToPageError = experimental_Agent.PagesAddLanguageToPageError;

type PagesAddLanguageToPageResponse = experimental_Agent.PagesAddLanguageToPageResponse;

type PagesAddLanguageToPageData = experimental_Agent.PagesAddLanguageToPageData;

type PagesCreatePageError = experimental_Agent.PagesCreatePageError;

type PagesCreatePageResponse = experimental_Agent.PagesCreatePageResponse;

type PagesCreatePageData = experimental_Agent.PagesCreatePageData;

type SitesGetSiteIdFromItemError = experimental_Agent.SitesGetSiteIdFromItemError;

type SitesGetSiteIdFromItemResponse = experimental_Agent.SitesGetSiteIdFromItemResponse;

type SitesGetSiteIdFromItemData = experimental_Agent.SitesGetSiteIdFromItemData;

type SitesGetAllPagesBySiteError = experimental_Agent.SitesGetAllPagesBySiteError;

type SitesGetAllPagesBySiteResponse = experimental_Agent.SitesGetAllPagesBySiteResponse;

type SitesGetAllPagesBySiteData = experimental_Agent.SitesGetAllPagesBySiteData;

type SitesGetSiteDetailsError = experimental_Agent.SitesGetSiteDetailsError;

type SitesGetSiteDetailsResponse = experimental_Agent.SitesGetSiteDetailsResponse;

type SitesGetSiteDetailsData = experimental_Agent.SitesGetSiteDetailsData;

type SitesGetSitesListError = experimental_Agent.SitesGetSitesListError;

type SitesGetSitesListResponse = experimental_Agent.SitesGetSitesListResponse;

type SitesGetSitesListData = experimental_Agent.SitesGetSitesListData;

export type Options<
  TData extends TDataShape = TDataShape,
  ThrowOnError extends boolean = boolean,
> = ClientOptions<TData, ThrowOnError> & {
  /**
   * You can provide a client instance returned by `createClient()` instead of
   * individual options. This might be also useful if you want to implement a
   * custom client.
   */
  client?: Client;
  /**
   * You can pass arbitrary values through the `meta` object. This can be
   * used to access values that aren't defined as part of the SDK function.
   */
  meta?: Record<string, unknown>;
};

/**
 * List sites
 * Retrieves a list of available sites with their basic information including name, display name, and URL.
 */
export const sitesGetSitesList = <ThrowOnError extends boolean = false>(
  options?: Options<SitesGetSitesListData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    SitesGetSitesListResponse,
    SitesGetSitesListError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/sites',
    ...options,
  });
};

/**
 * Retrieve the site details
 * Retrieves the details of a specific site including its  ID, name, and root path.
 */
export const sitesGetSiteDetails = <ThrowOnError extends boolean = false>(
  options: Options<SitesGetSiteDetailsData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    SitesGetSiteDetailsResponse,
    SitesGetSiteDetailsError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/sites/{siteId}',
    ...options,
  });
};

/**
 * List pages of a site
 * Retrieves a list of pages for a specific site, including each page's ID and path. You can optionally filter the pages by language.
 */
export const sitesGetAllPagesBySite = <ThrowOnError extends boolean = false>(
  options: Options<SitesGetAllPagesBySiteData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    SitesGetAllPagesBySiteResponse,
    SitesGetAllPagesBySiteError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/sites/{siteName}/pages',
    ...options,
  });
};

/**
 * Retrieve a site ID from an item ID
 * Retrieves the site ID associated with a specific item ID. This is useful for determining which site an item belongs to.
 */
export const sitesGetSiteIdFromItem = <ThrowOnError extends boolean = false>(
  options: Options<SitesGetSiteIdFromItemData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    SitesGetSiteIdFromItemResponse,
    SitesGetSiteIdFromItemError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/sites/site-id-from-item/{itemId}',
    ...options,
  });
};

/**
 * Create a page
 * Creates a new page using the specified template under the parent page. You can set field values and the language for the new page.
 */
export const pagesCreatePage = <ThrowOnError extends boolean = false>(
  options: Options<PagesCreatePageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PagesCreatePageResponse,
    PagesCreatePageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/create',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Add a language version to a page
 * Adds a new language version for an existing page. This allows you to have the same page content available in multiple languages.
 */
export const pagesAddLanguageToPage = <ThrowOnError extends boolean = false>(
  options: Options<PagesAddLanguageToPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PagesAddLanguageToPageResponse,
    PagesAddLanguageToPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/add-language',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Retrieve components on a page
 * Retrieves a list of components that are currently added to a specific page. Supply an optional variantId to resolve component content against a specific personalization or A/B testing variant.
 */
export const pagesGetComponentsOnPage = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetComponentsOnPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetComponentsOnPageResponse,
    PagesGetComponentsOnPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/components',
    ...options,
  });
};

/**
 * Add a component to a page
 * Adds a component to a specific placeholder on a page. You can specify the component type, placeholder location, and configure its initial settings. Optionally, you can specify its position relative to an existing component.
 */
export const pagesAddComponentOnPage = <ThrowOnError extends boolean = false>(
  options: Options<PagesAddComponentOnPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PagesAddComponentOnPageResponse,
    PagesAddComponentOnPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/components',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Set the component datasource
 * Sets the datasource for a specific component on a page. Supply an optional variantId in the request body to update the datasource for a specific personalization variant rule rather than the base rendering.
 */
export const pagesSetComponentDatasource = <ThrowOnError extends boolean = false>(
  options: Options<PagesSetComponentDatasourceData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PagesSetComponentDatasourceResponse,
    PagesSetComponentDatasourceError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/components/{componentId}/datasource',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Search for pages on a site
 * Searches for all  pages in a specific site using a search term that matches page titles and content. The response returns the matching pages with their details including the page ID, path, and name.
 */
export const pagesSearchSite = <ThrowOnError extends boolean = false>(
  options: Options<PagesSearchSiteData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesSearchSiteResponse,
    PagesSearchSiteError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/search',
    ...options,
  });
};

/**
 * Retrieve the page path by live URL
 * Retrieves the page item path that corresponds to a live URL on a website.
 */
export const pagesGetPagePathByLiveUrl = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPagePathByLiveUrlData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPagePathByLiveUrlResponse,
    PagesGetPagePathByLiveUrlError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/path-by-url',
    ...options,
  });
};

/**
 * Retrieve the screenshot of a page
 * Retrieves the screenshot of a specific page. This endpoint captures and returns a base64-encoded image of the live page. Supply an optional variantId to capture the screenshot for a specific personalization or A/B testing variant.
 */
export const pagesGetPageScreenshot = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPageScreenshotData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPageScreenshotResponse,
    PagesGetPageScreenshotError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/screenshot',
    ...options,
  });
};

/**
 * Retrieve the page HTML
 * Retrieves the HTML content of a specific page. This endpoint returns the raw HTML of the page as it would appear in the browser. Supply an optional variantId to retrieve the HTML for a specific personalization or A/B testing variant.
 */
export const pagesGetPageHtml = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPageHtmlData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPageHtmlResponse,
    PagesGetPageHtmlError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/html',
    ...options,
  });
};

/**
 * Retrieve the page preview URL
 * Retrieves the preview URL of a specific page. This endpoint returns the URL that can be used to preview the page. Supply an optional variantId to obtain a preview URL scoped to a specific personalization or A/B testing variant.
 */
export const pagesGetPagePreviewUrl = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPagePreviewUrlData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPagePreviewUrlResponse,
    PagesGetPagePreviewUrlError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/preview-url',
    ...options,
  });
};

/**
 * Retrieve the page template by ID
 * Retrieves the details of a specific page template, including its fields and settings. Use this endpoint to understand the structure and available fields of a template before creating a page.
 */
export const pagesGetPageTemplateById = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPageTemplateByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPageTemplateByIdResponse,
    PagesGetPageTemplateByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/template-by-id',
    ...options,
  });
};

/**
 * Retrieve the page details
 * Retrieves the details about a page including its ID, name, and path location. This endpoint provides the information you need to understand and modify the page's structure.
 */
export const pagesGetPage = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetPageResponse,
    PagesGetPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}',
    ...options,
  });
};

/**
 * List allowed components for a placeholder
 * Retrieves the list of components that can be added to a specific placeholder on a page. This ensures only compatible components are used in each placeholder. You can use * to fetch all components. Supply an optional variantId to resolve allowed components against a specific personalization or A/B testing variant.
 */
export const pagesGetAllowedComponentsByPlaceholder = <ThrowOnError extends boolean = false>(
  options: Options<PagesGetAllowedComponentsByPlaceholderData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PagesGetAllowedComponentsByPlaceholderResponse,
    PagesGetAllowedComponentsByPlaceholderError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/pages/{pageId}/placeholders/{placeholderName}/allowed-components',
    ...options,
  });
};

/**
 * Create content item
 * Creates a new content item using the specified template and field values.
 */
export const contentCreateContentItem = <ThrowOnError extends boolean = false>(
  options: Options<ContentCreateContentItemData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    ContentCreateContentItemResponse,
    ContentCreateContentItemError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content/create',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Delete a content item
 * Deletes a content item and optionally all of its child items.
 */
export const contentDeleteContent = <ThrowOnError extends boolean = false>(
  options: Options<ContentDeleteContentData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).delete<
    ContentDeleteContentResponse,
    ContentDeleteContentError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content/{itemId}',
    ...options,
  });
};

/**
 * Retrieve a content item by ID
 * Retrieves the details of a specific content item by specifying its ID.
 */
export const contentGetContentItemById = <ThrowOnError extends boolean = false>(
  options: Options<ContentGetContentItemByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ContentGetContentItemByIdResponse,
    ContentGetContentItemByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content/{itemId}',
    ...options,
  });
};

/**
 * Update a content item
 * Updates an existing content item, including its fields and language.
 */
export const contentUpdateContent = <ThrowOnError extends boolean = false>(
  options: Options<ContentUpdateContentData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).put<
    ContentUpdateContentResponse,
    ContentUpdateContentError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content/{itemId}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Retrieve a content item by path
 * Retrieves the details of a content item by specifying its path in the content tree.
 */
export const contentGetContentItemByPath = <ThrowOnError extends boolean = false>(
  options: Options<ContentGetContentItemByPathData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ContentGetContentItemByPathResponse,
    ContentGetContentItemByPathError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content',
    ...options,
  });
};

/**
 * List insert options
 * Retrieves a list of content templates that can be inserted as child items under the specified parent item.
 */
export const contentListAvailableInsertoptions = <ThrowOnError extends boolean = false>(
  options: Options<ContentListAvailableInsertoptionsData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ContentListAvailableInsertoptionsResponse,
    ContentListAvailableInsertoptionsError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/content/{itemId}/insert-options',
    ...options,
  });
};

/**
 * Create a component datasource
 * Creates a new datasource item for a specific component using the provided data field values. The datasource will be created in the appropriate location based on the component's configuration.
 */
export const componentsCreateComponentDatasource = <ThrowOnError extends boolean = false>(
  options: Options<ComponentsCreateComponentDatasourceData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    ComponentsCreateComponentDatasourceResponse,
    ComponentsCreateComponentDatasourceError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/components/{componentId}/datasources',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Search for component datasources
 * Searches for available datasources that can be used with a specific component. This helps you find existing content to use as datasources.
 */
export const componentsSearchComponentDatasources = <ThrowOnError extends boolean = false>(
  options: Options<ComponentsSearchComponentDatasourcesData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ComponentsSearchComponentDatasourcesResponse,
    ComponentsSearchComponentDatasourcesError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/components/{componentId}/datasources/search',
    ...options,
  });
};

/**
 * List components
 * Retrieves a list of components available for a specific site, including both built-in components and custom components that can be used on pages.
 */
export const componentsListComponents = <ThrowOnError extends boolean = false>(
  options: Options<ComponentsListComponentsData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ComponentsListComponentsResponse,
    ComponentsListComponentsError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/components',
    ...options,
  });
};

/**
 * Retrieve the component details
 * Retrieves the details of a specific component, including its ID, name, and datasource options.
 */
export const componentsGetComponent = <ThrowOnError extends boolean = false>(
  options: Options<ComponentsGetComponentData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    ComponentsGetComponentResponse,
    ComponentsGetComponentError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/components/{componentId}',
    ...options,
  });
};

/**
 * Upload an asset
 * Uploads a new digital asset to the system and stores it with the provided metadata.
 */
export const assetsUploadAsset = <ThrowOnError extends boolean = false>(
  options: Options<AssetsUploadAssetData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    AssetsUploadAssetResponse,
    AssetsUploadAssetError,
    ThrowOnError
  >({
    ...formDataBodySerializer,
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/assets/upload',
    ...options,
    headers: {
      'Content-Type': null,
      ...options?.headers,
    },
  });
};

/**
 * Search for assets
 * Searches for digital assets such as videos, images, and documents using query terms, file types, or tags.
 */
export const assetsSearchAssets = <ThrowOnError extends boolean = false>(
  options?: Options<AssetsSearchAssetsData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    AssetsSearchAssetsResponse,
    AssetsSearchAssetsError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/assets/search',
    ...options,
  });
};

/**
 * Retrieve the asset details
 * Retrieves the details of a specific digital asset by specifying its ID.
 */
export const assetsGetAssetInformation = <ThrowOnError extends boolean = false>(
  options: Options<AssetsGetAssetInformationData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    AssetsGetAssetInformationResponse,
    AssetsGetAssetInformationError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/assets/{assetId}',
    ...options,
  });
};

/**
 * Update an asset
 * Updates the metadata and properties of an existing digital asset, such as alt text, description, and tags.
 */
export const assetsUpdateAsset = <ThrowOnError extends boolean = false>(
  options: Options<AssetsUpdateAssetData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).put<
    AssetsUpdateAssetResponse,
    AssetsUpdateAssetError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/assets/{assetId}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * List languages
 * Retrieves all languages available.
 */
export const environmentsListLanguages = <ThrowOnError extends boolean = false>(
  options?: Options<EnvironmentsListLanguagesData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    EnvironmentsListLanguagesResponse,
    EnvironmentsListLanguagesError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/environments/languages',
    ...options,
  });
};

/**
 * Create component A/B experiment
 * Creates a new A/B test for a specific component on a page. The test will allow you to compare different versions of the component to determine which performs better.
 */
export const experimentsCreateComponentAbTest = <ThrowOnError extends boolean = false>(
  options: Options<ExperimentsCreateComponentAbTestData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    ExperimentsCreateComponentAbTestResponse,
    ExperimentsCreateComponentAbTestError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/experiments/flows',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Update A/B experiment
 * Updates an existing A/B experiment.
 */
export const experimentsUpdateAbTest = <ThrowOnError extends boolean = false>(
  options: Options<ExperimentsUpdateAbTestData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).put<
    ExperimentsUpdateAbTestResponse,
    ExperimentsUpdateAbTestError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/experiments/{flowId}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * @deprecated
 * Create a personalization variant
 * Creates a new personalization variant of a page using a single condition template and parameters.
 */
export const personalizationCreatePersonalizationVersionV1 = <ThrowOnError extends boolean = false>(
  options: Options<PersonalizationCreatePersonalizationVersionV1Data, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PersonalizationCreatePersonalizationVersionV1Response,
    PersonalizationCreatePersonalizationVersionV1Error,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/{pageId}/versions',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Create a personalization variant
 * Creates a new personalization variant of a page, enabling you to define targeting rules for different audiences.
 */
export const personalizationCreatePersonalizationVersion = <ThrowOnError extends boolean = false>(
  options: Options<PersonalizationCreatePersonalizationVersionData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PersonalizationCreatePersonalizationVersionResponse,
    PersonalizationCreatePersonalizationVersionError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v2/personalization/{pageId}/versions',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * @deprecated
 * Get personalization versions by page
 * Retrieves all personalization versions configured for a specific page, including their targeting rules and content variations.
 */
export const personalizationGetPersonalizationVersionsByPageV1 = <
  ThrowOnError extends boolean = false,
>(
  options: Options<PersonalizationGetPersonalizationVersionsByPageV1Data, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PersonalizationGetPersonalizationVersionsByPageV1Response,
    PersonalizationGetPersonalizationVersionsByPageV1Error,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/by-page/{pageId}',
    ...options,
  });
};

/**
 * List personalization variants by page
 * Retrieves all personalization variants defined for a specific page, including IDs, names, creation dates, and associated variants.
 */
export const personalizationGetPersonalizationVersionsByPage = <
  ThrowOnError extends boolean = false,
>(
  options: Options<PersonalizationGetPersonalizationVersionsByPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PersonalizationGetPersonalizationVersionsByPageResponse,
    PersonalizationGetPersonalizationVersionsByPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v2/personalization/by-page/{pageId}',
    ...options,
  });
};

/**
 * List personalization condition templates
 * Retrieves all available condition templates for personalization.
 */
export const personalizationGetConditionTemplates = <ThrowOnError extends boolean = false>(
  options?: Options<PersonalizationGetConditionTemplatesData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    PersonalizationGetConditionTemplatesResponse,
    PersonalizationGetConditionTemplatesError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/condition-templates',
    ...options,
  });
};

/**
 * Retrieve a personalization condition template
 * Retrieves a condition template by ID including its parameters for creating a personalization variant on a page.
 */
export const personalizationGetConditionTemplateById = <ThrowOnError extends boolean = false>(
  options: Options<PersonalizationGetConditionTemplateByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    PersonalizationGetConditionTemplateByIdResponse,
    PersonalizationGetConditionTemplateByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/condition-templates/{template_id}',
    ...options,
  });
};

/**
 * Update a personalization version for a page
 * Updates the audience name, variant name, and targeting rules of an existing personalization version.
 */
export const personalizationUpdatePersonalizationVersion = <ThrowOnError extends boolean = false>(
  options: Options<PersonalizationUpdatePersonalizationVersionData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).put<
    PersonalizationUpdatePersonalizationVersionResponse,
    PersonalizationUpdatePersonalizationVersionError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/{pageId}/versions/{variantId}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Hide a component on the default page variant
 * Hides a component on the default page variant by adding a personalization rule with HideRenderingAction. This endpoint only works with the Default variant.
 */
export const personalizationHideComponentOnDefaultPage = <ThrowOnError extends boolean = false>(
  options: Options<PersonalizationHideComponentOnDefaultPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    PersonalizationHideComponentOnDefaultPageResponse,
    PersonalizationHideComponentOnDefaultPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/personalization/{pageId}/components/{componentId}/hide',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * List flow definitions for a page
 * Retrieves the flow definitions for a specific page, including the structure, variants, and configuration details.
 */
export const flowsListFlowDefinitionsByPage = <ThrowOnError extends boolean = false>(
  options: Options<FlowsListFlowDefinitionsByPageData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    FlowsListFlowDefinitionsByPageResponse,
    FlowsListFlowDefinitionsByPageError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/flows/by-page/{pageId}',
    ...options,
  });
};

/**
 * Get flow definition
 * Retrieves the flow definition for a specific flow, including the structure, variants, and configuration details.
 */
export const flowsGetFlowDefinition = <ThrowOnError extends boolean = false>(
  options: Options<FlowsGetFlowDefinitionData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    FlowsGetFlowDefinitionResponse,
    FlowsGetFlowDefinitionError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/flows/{flowId}',
    ...options,
  });
};

/**
 * Reset component variant
 * Resets all personalization and A/B test configurations for a component variant in a flow. Provide pageId, componentId, and variantId in the request body. When variantId is omitted, resets the Default variant.
 */
export const flowsResetComponentVariant = <ThrowOnError extends boolean = false>(
  options: Options<FlowsResetComponentVariantData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    FlowsResetComponentVariantResponse,
    FlowsResetComponentVariantError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/flows/{flowId}/variants/reset',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Get flow definition variant details
 * Retrieves the details of a specific variant for a given flow, including its datasource and components.
 */
export const flowsGetVariant = <ThrowOnError extends boolean = false>(
  options: Options<FlowsGetVariantData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    FlowsGetVariantResponse,
    FlowsGetVariantError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/flows/{flowId}/variants/{variantId}',
    ...options,
  });
};

/**
 * Set up flow definition variant
 * Sets up a specific variant for a given flow using the provided variant strategy.
 */
export const flowsSetupVariant = <ThrowOnError extends boolean = false>(
  options: Options<FlowsSetupVariantData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    FlowsSetupVariantResponse,
    FlowsSetupVariantError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/flows/{flowId}/variants/{variantId}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * List brand kits
 * Retrieves all brand kits available within an organization.
 */
export const brandkitsListBrandkits = <ThrowOnError extends boolean = false>(
  options?: Options<BrandkitsListBrandkitsData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    BrandkitsListBrandkitsResponse,
    BrandkitsListBrandkitsError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brandkits',
    ...options,
  });
};

/**
 * Retrieve a brand kit by ID
 * Retrieves a brand kit by ID, including its sections and associated fields (subsections).
 */
export const brandkitsGetBrandkitById = <ThrowOnError extends boolean = false>(
  options: Options<BrandkitsGetBrandkitByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    BrandkitsGetBrandkitByIdResponse,
    BrandkitsGetBrandkitByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brandkits/{brandkitId}',
    ...options,
  });
};

/**
 * List brief types
 * Retrieves a list of all available brief types, including their metadata and field definitions such as field types, labels, validation rules, and other information used by AI when generating briefs.
 */
export const listBriefTypes = <ThrowOnError extends boolean = false>(
  options?: Options<ListBriefTypesData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<
    ListBriefTypesResponse2,
    ListBriefTypesError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief/brief-types',
    ...options,
  });
};

/**
 * List Briefs
 * Retrieves a paginated list of all briefs.
 */
export const listBriefs = <ThrowOnError extends boolean = false>(
  options?: Options<ListBriefsData, ThrowOnError>,
) => {
  return (options?.client ?? _heyApiClient).get<ListBriefsResponse2, ListBriefsError, ThrowOnError>(
    {
      security: [
        {
          scheme: 'bearer',
          type: 'http',
        },
      ],
      url: '/api/v1/brief',
      ...options,
    },
  );
};

/**
 * Create a brief
 * Creates a brief using the specified brief type ID, locale, and provided data fields. The new brief is saved as a draft in the Brief management tool.
 */
export const briefCreateBrief = <ThrowOnError extends boolean = false>(
  options: Options<BriefCreateBriefData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    BriefCreateBriefResponse,
    BriefCreateBriefError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Generate a brief
 * Generates a brief and the content of its fields, using the specified brief type, brand kit, and prompt. This endpoint does not save the generated brief. You can copy the response and use the Create a brief endpoint to save it in the Brief management tool.
 */
export const briefsGenerateBrief = <ThrowOnError extends boolean = false>(
  options: Options<BriefsGenerateBriefData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    BriefsGenerateBriefResponse,
    BriefsGenerateBriefError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief/generate',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * @deprecated
 * Generate a brief
 * Generates a brief and the content of its fields, using the specified brief type, brand kit, and prompt. This endpoint does not save the generated brief. You can copy the response and use the Create a brief endpoint to save it in the Brief management tool.
 * This path is kept for backward compatibility and will be removed on 2026-09-20. Please migrate to `POST /api/v1/brief/generate`.
 */
export const briefsGenerateBriefDeprecated = <ThrowOnError extends boolean = false>(
  options: Options<BriefsGenerateBriefDeprecatedData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<
    BriefsGenerateBriefDeprecatedResponse,
    BriefsGenerateBriefDeprecatedError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/briefs/generate',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Get Brief Type by ID
 * Retrieves a single brief type by its ID, including its field definitions and audit metadata.
 */
export const getBriefTypeById = <ThrowOnError extends boolean = false>(
  options: Options<GetBriefTypeByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    GetBriefTypeByIdResponse,
    GetBriefTypeByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief/brief-types/{brief_type_id}',
    ...options,
  });
};

/**
 * Get Brief by ID
 * Retrieves a single brief by its ID, including its fields, type reference, and audit metadata.
 */
export const getBriefById = <ThrowOnError extends boolean = false>(
  options: Options<GetBriefByIdData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<
    GetBriefByIdResponse,
    GetBriefByIdError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief/{brief_id}',
    ...options,
  });
};

/**
 * Update Brief
 * Updates an existing brief by its ID.
 */
export const briefUpdateBrief = <ThrowOnError extends boolean = false>(
  options: Options<BriefUpdateBriefData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).put<
    BriefUpdateBriefResponse,
    BriefUpdateBriefError,
    ThrowOnError
  >({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/brief/{brief_id}',
    ...options,
    headers: {
      'Content-Type': 'application/json',
      ...options?.headers,
    },
  });
};

/**
 * Revert a job
 * Reverts the operations linked to a specific job ID, restoring the state prior to execution. This is useful for undoing changes made by automated processes.
 */
export const jobsRevertJob = <ThrowOnError extends boolean = false>(
  options: Options<JobsRevertJobData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).post<unknown, JobsRevertJobError, ThrowOnError>({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/jobs/{jobId}/revert',
    ...options,
  });
};

/**
 * Retrieve job details
 * Retrieves the details of a specific job.
 */
export const jobsGetJob = <ThrowOnError extends boolean = false>(
  options: Options<JobsGetJobData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<unknown, JobsGetJobError, ThrowOnError>({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/jobs/{jobId}',
    ...options,
  });
};

/**
 * List job operations
 * Retrieves a list of actions associated with a specific job ID, including operation type, status, and timestamps, to support tracing and potential reversion of those actions.
 */
export const jobsListOperations = <ThrowOnError extends boolean = false>(
  options: Options<JobsListOperationsData, ThrowOnError>,
) => {
  return (options.client ?? _heyApiClient).get<unknown, JobsListOperationsError, ThrowOnError>({
    security: [
      {
        scheme: 'bearer',
        type: 'http',
      },
    ],
    url: '/api/v1/jobs/{jobId}/operations',
    ...options,
  });
};
