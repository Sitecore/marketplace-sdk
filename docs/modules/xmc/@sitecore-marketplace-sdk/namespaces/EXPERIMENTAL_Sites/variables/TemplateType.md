[**@sitecore-marketplace-sdk/xmc**](../../../../README.md)

***

[@sitecore-marketplace-sdk/xmc](../../../../README.md) / [experimental\_Sites](../README.md) / TemplateType

# Variable: TemplateType

> **TemplateType**: `object`

Defined in: [packages/xmc/src/experimental/client-sites/types.gen.ts:1323](https://github.com/Sitecore/marketplace-sdk/blob/main/packages/xmc/src/experimental/client-sites/types.gen.ts#L1323)

The type of template.

## Type Declaration

### BRANCH\_TEMPLATE

> `readonly` **BRANCH\_TEMPLATE**: `"BranchTemplate"` = `'BranchTemplate'`

### ITEM\_TEMPLATE

> `readonly` **ITEM\_TEMPLATE**: `"ItemTemplate"` = `'ItemTemplate'`
